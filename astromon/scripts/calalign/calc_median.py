...
def filter_xcorr(xcorr, *, snr=None, r_angle=None, start=None, stop=None, **kwargs):
...
def get_parser():
    ...
    parser.add_argument(
        "--out", type=Path, help="JSON filename where to write offsets."
    )
    parser.add_argument(
        "--bins-per-year", default=2, help="Number of bins per year. Default=2"
    )
    parser.add_argument(
        "--snr", default=5, help="Minimum SNR to consider an x-ray source"
    )
    parser.add_argument("--plot", action="store_true", default=False, help="Show plots")
    parser.add_argument(
        "--stdout", action="store_true", default=False, help="Write offsets to stdout."
    )
    return parser
...
def main():
    ...
    years_dy_binned, dy_binned, dy_bins = binned_median(
        matches["year"], matches["dy"], bins_per_year=args.bins_per_year
    )
    ...
    matches["dy_sub"] = matches["dy"] - np.interp(
        x=matches["year"], xp=medians["years_dy"], fp=medians["dy"]
    )
    matches["dz_sub"] = matches["dz"] - np.interp(
        x=matches["year"], xp=medians["years_dz"], fp=medians["dz"]
    )
    ...
    shifts = [
        ...
        for row in medians[
            (medians["stop"] <= args.stop) & (medians["start"] > args.start)
        ]
    ]
