...
def apply_calalign_shift(tstart, tstop, in_file, out_file, dy, dz, date, clobber=False):
    ...
    for row in hdus[1].data:
        ...
        Rz = np.array([[cz, 0.0, -sz],
                       [0.0, 1.0, 0.0],
                       [sz, 0.0, cz]])
        Ry = np.array([[cy, -sy, 0.0],
                       [sy, cy, 0.0],
                       [0.0, 0.0, 1.0]])
        R = Rz @ Ry
        ...
        msg = f'{row["INSTR_ID"].strip()} alignment shift (dy, dz) = ({dy:.2f}, {dz:.2f})'
        # print(msg)
...
def get_parser():
    ...
    parser.add_argument(
        '--version',
        default='N0010',
        help='The new file version. Default: N0010.'
    )
    parser.add_argument(
        '--base',
        type=Path,
...
def main():
    ...
    m = re.match(r'pcad(?P<date>D[0-9\-]+)align(?P<version>N[0-9]+).fits', args.base.name)
    update_tstop(
        in_file=args.base,
        tstop=CxoTime(offsets[0]["tstart"], scale='tt'),
        out_file=args.out_dir / args.base.name.replace(m.groupdict()['version'], args.version),
        date=date,
        clobber=args.clobber
    )
