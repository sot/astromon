...
def do_events():  # noqa: PLR0915
    ...
    for obsdir in tqdm(obsdirs[::-1]):
        ...
        if not obspar_files:
            ...
            continue
        obsid_info = {r[0]: r[3] for r in ascii.read(obspar_files[0])}
        det = obsid_info["detector"].lower()
        # print(obsid, det)

        orig_asol_files = list(
            (obsdir / "archive" / "uncorr" / "out1").glob("pcad*asol*")
        )
        ...
        orig_ys, orig_zs = radec_to_yagzag(
            orig_hdu[1].data["ra"], orig_hdu[1].data["dec"], q
        )
        new_ys, new_zs = radec_to_yagzag(
            corr_hdu[1].data["ra"], corr_hdu[1].data["dec"], q
        )
        ...
        sky_0 = {
            ...
            for k, v in orig_evt_hdu[1].header.items()
            if k.startswith("TTYPE") and v in ("x", "y")
        }
        sky_x_0 = sky_0["x"]
        sky_y_0 = sky_0["y"]
        ...
        obsinfo = {
            ...
            "sol_ra_change_mean": np.mean(
                corr_hdu[1].data["ra"] - orig_hdu[1].data["ra"]
            ),
            "sol_dec_change_mean": np.mean(
                corr_hdu[1].data["dec"] - orig_hdu[1].data["dec"]
            ),
