...
def celldetect(evt, src, si, asol):
    CIAO = Ciao(
        workdir=evt.parent,
        logger=pyyaks.logger.get_logger('ciao', level='WARNING')
    )
...
def main():
    ...
    for _, obsid, i in obsids_to_check.OBS:
        try:
            obsdir = Path('reprodata').absolute() / f'obs{obsid//1000:02d}' / f'{obsid:05d}'
            obsdir.mkdir(parents=True, exist_ok=True)
            CIAO = Ciao(
                workdir=obsdir,
                logger=pyyaks.logger.get_logger('ciao', level='INFO')
            )
            ...
            with chdir(obsdir):
                ...
                if len(new_asols) < 1:
                    ...
                    run_cmd(
                        "python", RUNASP, "--obsid", str(obsid), "--param", f"align={align}",
                        "--dir", "archive"
                    )
                    new_asols = list(obsdir.glob('archive/ASP*/out1/*asol*'))
                ...
                if len(old_asols) < 1:
                    ...
                    run_cmd(
                        "python", RUNASP, "--obsid", str(obsid), "--label", "uncorr",
                        "--dir", "archive"
                    )
                    old_asols = list(obsdir.glob('archive/uncorr/out1/*asol*'))
            ...
            obsid_info = {
                r[0]: r[3] for r in ascii.read(list(obsdir.glob('archive/obspar/*obs0a.par'))[0])
            }
            ...
            if not uncorr_evt2.exists():
                ...
                CIAO("reproject_events", evt, uncorr_evt2, match="none", aspect=old_asol)
