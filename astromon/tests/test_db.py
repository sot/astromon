...
def test_save_explicit_replace_keys_also_replaces_pre_migration_rows():
    """An explicit replace_keys containing detect_method must get the same
    legacy-blank normalization as the auto-detected-key path.

    save()'s auto-detected key path treats a blank pre-migration detect_method
    (b"", backfilled by _cast_to_dtype) as "celldetect" so old rows get
    replaced instead of surviving alongside a fresh write -- see
    test_save_detect_method_key_replaces_pre_migration_rows above. Passing the
    same (obsid, detect_method) key explicitly via replace_keys is documented
    as an equivalent, supported way to key the same replacement, and must
    behave the same way.
    """
    import tables as tb
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        dbfile = Path(tmpdir) / "detect_method_replace_keys_test.h5"
        ...
        db.save(
            ...
            new_row,
            dbfile,
            replace_keys=("obsid", "detect_method"),
        )

        result = db.get_table("astromon_xray_src", dbfile)

    obsid_rows = result[result["obsid"] == 3334]
    assert len(obsid_rows) == 1, (
        "pre-migration row with blank detect_method must be replaced via an "
        f"explicit replace_keys too: got {len(obsid_rows)} rows"
    )
    assert float(obsid_rows["snr"][0]) == 9.0


def test_save_gaussian_only_does_not_delete_pre_migration_celldetect_rows():
    """A blank pre-migration detect_method row means "celldetect", not "any
    method" -- writing a *different* method for the same obsid must leave it
    alone.

    _cast_to_dtype backfills pre-migration rows with detect_method=b"", and
    that blank must be treated as celldetect for key-matching so a real
    celldetect save can still replace it (see the test above). But saving
    gaussian_detect data for the same obsid is not superseding celldetect
    data -- both should coexist, exactly as (obsid, detect_method) keying
    intends for any two real methods.
    """
    import tables as tb
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        dbfile = Path(tmpdir) / "detect_method_gaussian_only_test.h5"
        ...
        new_row["id"] = 2
        new_row["snr"] = 9.0
        new_row["detect_method"] = "gaussian_detect"
        db.save("astromon_xray_src", new_row, dbfile)

        result = db.get_table("astromon_xray_src", dbfile)

    obsid_rows = result[result["obsid"] == 4444]
    assert len(obsid_rows) == 2, (
        "a gaussian_detect-only save must not delete the pre-migration "
        f"celldetect row: got {len(obsid_rows)} rows"
    )
    detect_methods = sorted(m.strip() for m in obsid_rows["detect_method"])
    assert detect_methods == ["", "gaussian_detect"]
...
def test_get_cross_matches_joins_legacy_blank_detect_method_rows():
    """A legacy blank detect_method on one table must still join a real value
    on the other.

    astromon_xcorr and astromon_xray_src are saved independently, so one
    table's rows for an obsid can carry a real detect_method (e.g.
    "celldetect") while the other's are still blank ("") from before the
    column existed. The keyed join in get_cross_matches() used to compare
    detect_method literally, so a blank xcorr row for an obsid whose
    xray_src row already has "celldetect" was silently dropped from the
    result instead of being paired with it.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        dbfile = Path(tmpdir) / "cross_matches_legacy.h5"

        obs_row = Table(np.zeros(1, dtype=db.ASTROMON_OBS_DTYPE))
        obs_row["obsid"] = 5555
        obs_row["date_obs"] = "2020-01-01T00:00:00"
        obs_row["category_id"] = 10
        db.save("astromon_obs", obs_row, dbfile)

        xray_row = Table(np.zeros(1, dtype=db.ASTROMON_XRAY_SRC_DTYPE))
        xray_row["obsid"] = 5555
        xray_row["id"] = 1
        xray_row["detect_method"] = "celldetect"
        db.save("astromon_xray_src", xray_row, dbfile)

        cat_row = Table(np.zeros(1, dtype=db.ASTROMON_CAT_SRC_DTYPE))
        cat_row["obsid"] = 5555
        cat_row["id"] = 1
        db.save("astromon_cat_src", cat_row, dbfile)

        # Legacy xcorr row: written before detect_method existed, so it is
        # backfilled to blank ("") rather than "celldetect".
        xcorr_row = Table(np.zeros(1, dtype=db.ASTROMON_XCORR_DTYPE))
        xcorr_row["select_name"] = "astromon_21"
        xcorr_row["obsid"] = 5555
        xcorr_row["c_id"] = 1
        xcorr_row["x_id"] = 1
        db.save("astromon_xcorr", xcorr_row, dbfile)

        result = db.get_cross_matches("astromon_21", dbfile)
...
def test_save_crash_while_writing_leaves_original_table_intact():
    """A process killed while save() writes the new data must not lose the old table.

    save() used to remove_node the existing table and then create_table the
    replacement as two separate steps, so a crash between them (or during the
    create_table call, which is the slow part for a large table) left the table
    missing entirely. save() now writes to a temporary node first and swaps it
    in, so a failure during that write leaves the original table untouched.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        dbfile = Path(tmpdir) / "crash_during_write.h5"
        _seeded_cat_src_db(dbfile)
        original = db.get_table("astromon_cat_src", dbfile)
        assert len(original) == 4

        with (
            patch.object(tb.File, "create_table", side_effect=RuntimeError("boom")),
