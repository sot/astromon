...
def get_cross_matches(name="astromon_21", dbfile=None, **kwargs):
    ...
    # Rows stored before detect_method existed were backfilled to "" (astromon_xcorr
    # and astromon_xray_src are saved independently, so one table's rows for an
    # obsid can carry a real value while the other's are still blank). "" meant
    # celldetect back when it was the only method, so normalize it to that here --
    # otherwise the keyed join below would literal-compare "" against "celldetect"
    # and silently drop the match instead of pairing them.
    for detect_method_table in (matches, astromon_xray_src):
        blank = detect_method_table["detect_method"] == ""
        detect_method_table["detect_method"][blank] = "celldetect"
