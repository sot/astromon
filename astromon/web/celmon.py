...
def plot_offsets_history(
    ...
    plt.sca(ax)
    plot_cxctime(matches['time'], matches['dy'], '.', label='dy', color='tab:blue', alpha=0.4)
    plot_cxctime(matches['time'], matches['dz'], '.', label='dz', color='tab:orange', alpha=0.4)
    if dy_median is not None:
        plot_cxctime(dy_median['time'], dy_median['median'], color='tab:blue', linewidth=3)
    if dz_median is not None:
        plot_cxctime(dz_median['time'], dz_median['median'], color='tab:orange', linewidth=3)
...
def plot_offsets_q_history(
    ...
    plot_cxctime(times, np.tile(dy_median['median'], (2, 1)), '-', linewidth=2, color='tab:blue')
    ...
    plot_cxctime(times, np.tile(dz_median['median'], (2, 1)), '-', linewidth=2, color='tab:orange')
...
def plot_cdf_3(
    ...
    for q in quantiles:
        ...
        if r > plt.xlim()[1]:
            continue
        plt.plot([0., r], [q, q], '--', color='b', linewidth=1)
        plt.plot([r, r], [0., q], '--', color='b', linewidth=1)
...
def binned_median(time, vals, bins_per_year=2):
    ...
    tga['q95'] = tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.95))['vals']
    tga['q05'] = tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.05))['vals']
    tga['q75'] = tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.75))['vals']
    tga['q25'] = tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.25))['vals']
    tga['sigma_minus'] = \
        tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.158))['vals']
    tga['sigma_plus'] = \
        tg[['vals']].groups.aggregate(functools.partial(np.quantile, q=0.842))['vals']
...
def create_figures_cal(outdir, snr=5, n_years=5, draw_median=True):
    ...
    dy_median = binned_median(
        matches['time'], matches['dy'], bins_per_year=2
    )
    dz_median = binned_median(
