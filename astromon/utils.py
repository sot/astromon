...
def get_calalign_offsets(all_matches, ref_calalign=None, calalign_dir=None):
    ...
    # Fail loudly (ValueError) on a version string with a non-numeric segment
    # rather than silently dropping it: a shorter tuple compares as "earlier"
    # against a full-length one regardless of what the dropped digits meant,
    # which would pick the wrong CalDB row instead of raising.
    match_w_cal["cdbv"] = [
    ...
    for key in match_id_keys:
        if not np.all(reference[key] == actual[key]):
    ...
    # input row order survives -- restore all_matches' own order instead of
    # requiring the caller to have pre-sorted it. match_id_keys uniquely
    # identifies a match (see the obsid/x_id/detect_method note above), so this
    # is a direct lookup, not a guess.
    if len(result) != len(all_matches):
        raise RuntimeError("len(result) != len(all_matches)")
    result_index = {
        key: i
        for i, key in enumerate(
            zip(*[np.asarray(result[k]) for k in match_id_keys], strict=True)
        )
    }
    order = [
        result_index[key]
        for key in zip(
            *[np.asarray(all_matches[k]) for k in match_id_keys], strict=True
        )
    ]
    result = result[order]
...
def get_rebased_offsets(all_matches, ref_matrix=None, calalign_dir=None):
    ...
    *version* label, which can be a much older matrix than the most recent one --
    ...
    # together -- vstack always puts the has-caldb rows first, so any input where
    # the two groups are interleaved (not already has-caldb-rows-then-no-caldb-rows)
    # came back in a different row order than `all_matches`, silently breaking the
    # "copy of all_matches" contract documented above.
