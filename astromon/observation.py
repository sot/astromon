...
class Observation:
    ...
    def _download_archive(self, ftypes):  # noqa: PLR0912
        ...
        subsequent calls are no-ops. An obsid whose ``repro/`` already exists (a
        prior run completed reprocessing) never needs its archive files back
        either, even if ``secondary/`` was since cleared.
        """
        secondary = self.workdir / "secondary"
        repro = self.file_path("repro")
        if secondary.exists() or repro.exists():
            logger.debug(
                f"{self} {secondary if secondary.exists() else repro} exists,"
                " skipping download"
            )
