...
class Observation:
    ...
    def get_evt2_info(self):
        ...
        info = {
            ...
            "sim_x": float(header["SIM_X"]) if "SIM_X" in header else float("nan"),
            "sim_y": float(header["SIM_Y"]) if "SIM_Y" in header else float("nan"),
            "sim_z": float(header["SIM_Z"]) if "SIM_Z" in header else float("nan"),
    ...
    def _archive_already_downloaded(self) -> bool:
        """Whether _download_archive can skip straight to a no-op.

        True only once the completion marker is present. ``repro/`` existing is
        not a substitute: cleanup_downloads() deliberately clears the marker
        and deletes the raw evt2 while leaving repro/ untouched, specifically so
        a later retry re-fetches it instead of repeating the exact "Missing
        input files for task make_images: events" failure that motivated
        clearing the marker in the first place (see cleanup_downloads' and
        _archive_download_marker's docstrings). Trusting repro/ here would
        silently defeat that retry path.
        """
    ...
    def _download_archive(self, ftypes):  # noqa: PLR0912
        ...
        cannot be (and :meth:`_archive_already_downloaded` for why ``repro/``
        existing is not a substitute either). Subsequent calls are then no-ops.
        """
        ...
        secondary = self.workdir / "secondary"
        marker = self._archive_download_marker()
        if self._archive_already_downloaded():
            return
    ...
    def is_selected(self):
        ...
        A cheap ocat-only prefilter runs first and skips:
        ...
        This comes from the Chandra ocat rather than the obspar: the obspar is a
        separately-versioned archive product that is not guaranteed to be in sync
        ...
        That "better than 99.99%" is not 100%, and none of the ocat fields above
        actually say whether the observation is a real pointed exposure rather than
        a slew: an obsid can have a usable instrument and, for ACIS, TE/no-duty-cycle
        readout, and still not be OBS_MODE=POINTING. So an observation that survives
        the ocat prefilter is downloaded and its own evt2 header -- the one
        processing itself reads -- is checked directly. This costs nothing beyond
        what an accepted observation already needs: `process()` downloads the same
        evt2 for the tasks it goes on to run.
        ...
        if instr.startswith("ACIS") and not (
            seq.get("mode", "") == "TE" and seq.get("d_cyc", "") == "N"
        ):
            return False

        self.download(["evt2"])
        return self.get_evt2_info()["obs_mode"] == "POINTING"
