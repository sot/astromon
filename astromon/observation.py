...
class Observation:
    ...
    def __init__(
        ...
        self.workdir = (
            Path(self.tmp.name if workdir is None else workdir).expanduser() / subdir / self.obsid
        )
        self.archive_dir = (
            (Path(archive_dir).expanduser() / subdir / self.obsid) if archive_dir else None
        )
    ...
    def get_obspar(self):
        ...
        if m:
            ...
            self._obsid_info['version'] = version[0] + version[1] / 100 + version[2] / 10000
        else:
        ...
        for ftype in ftypes:
            if ftype == 'obspar':
                ...
                if ftype not in locs:
                    logger.error(f'{self} {ftype=} skipped because it is not in known locations')
                    continue
                src, dest = locs[ftype]
    ...
    def filter_events(self, radius=180, psfratio=1):  # radius in arcsec
        ...
        self.ciao('dmcoords', evt, op='cel', celfmt='deg', ra=ra, dec=dec, logging_tag=str(self))
    ...
    def calculate_centroids(self):
        ...
        for row in src:
            ...
            x, y = np.array(self.ciao.pget(
                'dmstat',
                'out_cntrd_phys',
                logging_tag=str(self)).split(',')
            ).astype(float)
            result.append([x, y])
    ...
    def get_sources(self, version='celldetect'):
        ...
        hdu_list = fits.open(
            self.workdir / 'sources' / f'{self.obsid}_{version}.src')
